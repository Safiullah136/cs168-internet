...
class StudentUSocket(StudentUSocketBase):
  ...
  def handle_accepted_payload(self, payload):
    ...
    self.rx_data += payload   # Appending payload to the end of rx_data which is in-order
    self.set_pending_ack()
  ...
  def maybe_send(self):
    ...
    while remaining > 0 and self.tx_data:
      payload_size = min(self.mss, remaining, len(self.tx_data))
